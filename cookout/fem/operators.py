...
class Operators1D(object):
    ...
    """
    def __init__(self, mesh, left_dirichlet_value=0.0,
    ...
    def _get_matrix(self, local_matrix_function):
        ...
        columns = list()
        values = list()
        node_numbers = np.zeros((self.mesh.element_order + 1,
                                 self.mesh.element_order + 1), dtype=np.int)
        for element in self.mesh.elements:
            width = self.mesh.nodes[element[-1]] - self.mesh.nodes[element[0]]
            local_matrix = local_matrix_function(self.mesh.element_order, width)
